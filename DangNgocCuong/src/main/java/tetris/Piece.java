...
public class Piece {
	...
	// <></>he algorithms to be done.
	private static final int NumberTPoint = 4;
	private TPoint[] body;
	private int[] skirt;
	private int width;
	private int height;
	...
	*/
	public Piece(TPoint[] points) {
		// YOUR CODE HERE
		this.body = points;
		for(int i=0;i<NumberTPoint;i++){
			width = max(points[i].x,width);
			height= max(points[i].y,height);
		}
		height++;
		width++;
		skirt = new int[width];
		for (int i=0;i<width;i++) {
			skirt[i]=5;
			for (int j = 0; j < NumberTPoint; j++)
				if (points[j].x == i)
					skirt[i] = min(points[j].y, skirt[i]);
		}
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		TPoint[] bodyofNewPiece = new TPoint[NumberTPoint];
		for(int i=0;i<NumberTPoint;i++) {
			bodyofNewPiece[i] = new TPoint(height-body[i].y-1,body[i].x) ;
		}
		//calculate bodyOfNewPiece TPoint array of the new piece
		// create a new Piece with bodyOfNewPiece
		Piece newpeiece = new Piece(bodyofNewPiece);
		return newpeiece;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
        Piece other_piece = root;
        for(int i=0;i<NumberTPoint;i++) {
            root.next = root.computeNextRotation();
            if(root.next.equals(other_piece)){
                root.next = other_piece;
                break;
            }
            root = root.next;
        }

		return other_piece; // YOUR CODE HERE
	}
